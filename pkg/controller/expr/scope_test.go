...
func TestScopeExpr(t *testing.T) {
	...
	actualScope, err := NewScope(nil, &types.Workflow{
...
func TestScopeOverride(t *testing.T) {
	...
	scope1, err := NewScope(nil, &types.Workflow{
		Metadata: &types.ObjectMetadata{
			Id:        "testWorkflow",
			CreatedAt: ptypes.TimestampNow(),
		},
	...
	scope2, err := NewScope(scope1, nil, nil)
	assert.NoError(t, err)
	...
	scope4, err := NewScope(scope1, nil, &types.WorkflowInvocation{
		Spec: &types.WorkflowInvocationSpec{
